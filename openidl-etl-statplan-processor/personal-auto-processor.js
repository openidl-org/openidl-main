...
module.exports.convertTextRecordsToJson = function (recordsText) {
	return convertTextRecordsToJsonUsingSchema(
		recordsText,
		premiumSchema,
		lossSchema
	);
...
function convertTextRecordsToJsonUsingSchema(
	recordsText,
	premiumSchema,
	lossSchema
) {
	...
	for (let record of records) {
		//transform record
		lcl_result = convertTextRecordToJsonUsingSchema(
			record,
			premiumSchema,
			lossSchema
		);
...
function convertTextRecordToJsonUsingSchema(record, premiumSchema, lossSchema) {
	...
	exports.process = async function (records) {
		...
		records.forEach((inputRecord) => {
			...
			//console.log(record)
			let jsonRecords = convertTextRecordsToJsonUsingSchema(
				record,
				premiumSchema,
				lossSchema
			);
