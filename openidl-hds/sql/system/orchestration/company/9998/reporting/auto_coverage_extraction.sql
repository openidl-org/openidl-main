create table openidl_ep_9998.tmp_pre_au_coverage as
    ...
    from openidl_base_9998.au_premium a)
    union all
    ...
    from openidl_base_9998.au_loss b);|create table openidl_ep_9998.tmp_au_coverage_ref as
    select reporting_code, reporting_name 
    from openidl_ep_9998.tmp_au_coverage 
    group by reporting_code, reporting_name 
    order by reporting_code::numeric ;|create table openidl_ep_9998.tmp_au_coverage as
    select 
           case when a.coverage_code = '1' then '1'
                  ...
                  a.*
    from openidl_ep_9998.tmp_pre_au_coverage a;|CREATE OR replace FUNCTION openidl_ep_9998.tmp_car_years(IN start_date date,IN end_date date, IN pv_reporting_code VARCHAR)
...
BEGIN
    ...
    RETURN cy;
END$$ language plpgsql;|CREATE OR replace FUNCTION openidl_ep_9998.tmp_au_earned_premium(IN start_date date,IN end_date date, IN pv_reporting_code VARCHAR)
    ...
    BEGIN
        ...
        RETURN ep;
    END$$ language plpgsql;|CREATE OR replace FUNCTION openidl_ep_9998.auto_outstanding(IN start_date date,IN end_date date, IN pv_coverage_code VARCHAR)
    returns      numeric AS $$DECLARE ep numeric;
    BEGIN
        ...
        SELECT Sum(x.loss_amount) outstanding_loss
            FROM  (SELECT t2.coverage_code,
                ...
                FROM   (SELECT coverage_code,
                        ...
                        openidl_ep_9998.tmp_au_coverage t2
        ...
        RETURN ep;
    END$$ language plpgsql;|CREATE OR replace FUNCTION openidl_ep_9998.tmp_au_incurred_loss(IN start_date date,IN end_date date, IN  pv_reporting_code VARCHAR)
    returns      numeric AS $$DECLARE ep numeric;
    BEGIN
        
        select sum(loss_amount) loss_amount
            from 
            ((select sum(loss_amount) loss_amount from openidl_ep_9998.tmp_au_coverage
        ...
        RETURN ep;
    END$$ language plpgsql;|CREATE OR replace FUNCTION openidl_ep_9998.tmp_au_incurred_count(IN start_date date,IN end_date date, IN pv_reporting_code VARCHAR)
    returns      numeric AS $$DECLARE ep numeric;
    BEGIN
        ...
        RETURN ep;
    END$$ language plpgsql;|select *, round(openidl_ep_9998.tmp_au_earned_premium('2000-01-01'::DATE, '2002-01-01'::DATE,a.reg_reporting_code)) earned_premium 
    ,round(openidl_ep_9998.tmp_car_years('2000-01-01'::DATE, '2002-01-01'::DATE,a.reg_reporting_code)) car_years
    ,round(openidl_ep_9998.tmp_au_incurred_count('2000-01-01'::DATE, '2002-01-01'::DATE,a.reg_reporting_code)) incurred_count
    ,round(openidl_ep_9998.tmp_au_incurred_loss('2000-01-01'::DATE, '2002-01-01'::DATE,a.reg_reporting_code)) incurred_loss
    from openidl_ep_9998.au_reg_reporting_ref a; |
    -- Temp Tables
    DROP TABLE openidl_ep_9998.tmp_pre_au_coverage;
    DROP TABLE openidl_ep_9998.tmp_au_coverage;

    --Functions (note should be torn down after each extraction);
    DROP FUNCTION openidl_ep_9998.tmp_car_years;
    DROP FUNCTION openidl_ep_9998.tmp_earned_premium;
    DROP FUNCTION openidl_ep_9998.tmp_incurred_loss;
    DROP FUNCTION openidl_ep_9998.tmp_incurred_count;
    DROP FUNCTION openidl_ep_9998.tmp_auto_outstanding;
