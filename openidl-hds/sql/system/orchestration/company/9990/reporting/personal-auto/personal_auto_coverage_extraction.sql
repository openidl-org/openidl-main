    ...
    DROP TABLE IF EXISTS openidl_ep_9990.tmp_au_coverage;
    DROP TABLE IF EXISTS openidl_ep_9990.tmp_au_coverage_ref;
    DROP TABLE IF EXISTS openidl_ep_9990.tmp_pre_au_coverage;
    DROP TABLE IF EXISTS openidl_ep_9990.tmp_au_pc;
    ...
    DROP FUNCTION IF EXISTS openidl_ep_9990.tmp_auto_outstanding;
    
    |
    create table openidl_ep_9990.tmp_au_pc as
    ...
    from openidl_base_9990.personal_auto_policy a)
    union all
    ...
    from openidl_base_9990.personal_auto_claim b);
    
    |
    create table openidl_ep_9990.tmp_au_coverage as
    select 
           case when a.coverage_code = '1' then '1'
                  ...
                  a.*
    from openidl_ep_9990.tmp_au_pc a;
    
    |
    create table openidl_ep_9990.tmp_au_coverage_ref as
    select reporting_code, reporting_name 
    from openidl_ep_9990.tmp_au_coverage 
    group by reporting_code, reporting_name 
    order by reporting_code::numeric ;
    
    |
CREATE OR replace FUNCTION openidl_ep_9990.tmp_car_years(IN start_date date,IN end_date date, IN pv_reporting_code VARCHAR)
...
|
    CREATE OR replace FUNCTION openidl_ep_9990.tmp_au_earned_premium(IN start_date date,IN end_date date, IN pv_reporting_code VARCHAR)
    ...
    |
    CREATE OR replace FUNCTION openidl_ep_9990.tmp_auto_outstanding(IN start_date date,IN end_date date, IN pv_coverage_code VARCHAR)
    returns      numeric AS $$DECLARE ep numeric;
    BEGIN
        ...
        SELECT Sum(x.loss_amount) outstanding_loss
            FROM  (SELECT t2.coverage_code,
                ...
                FROM   (SELECT coverage_code,
                        ...
                        openidl_ep_9990.tmp_au_coverage t2
    ...
    |
    CREATE OR replace FUNCTION openidl_ep_9990.tmp_au_incurred_loss(IN start_date date,IN end_date date, IN  pv_reporting_code VARCHAR)
    returns      numeric AS $$DECLARE ep numeric;
    BEGIN
        
        select sum(loss_amount) loss_amount
            from 
            ((select sum(loss_amount) loss_amount from openidl_ep_9990.tmp_au_coverage
            where transaction_code = '2'
            and reporting_code =  pv_reporting_code
            and accident_date > start_date
            and accident_date < end_date
            union
            (select openidl_ep_9990.tmp_auto_outstanding(start_date, end_date,'1') loss_amount))) a into ep; 
        RETURN ep;
    END$$ language plpgsql;
    
    |
    CREATE OR replace FUNCTION openidl_ep_9990.tmp_au_incurred_count(IN start_date date,IN end_date date, IN pv_reporting_code VARCHAR)
    returns      numeric AS $$DECLARE ep numeric;
    BEGIN
    ...
    |
    select *, round(openidl_ep_9990.tmp_au_earned_premium('2000-01-01'::DATE, '2002-01-01'::DATE,a.reporting_code)) earned_premium 
    ,round(openidl_ep_9990.tmp_car_years('2000-01-01'::DATE, '2002-01-01'::DATE,a.reporting_code)) car_years
    ,round(openidl_ep_9990.tmp_au_incurred_count('2000-01-01'::DATE, '2002-01-01'::DATE,a.reporting_code)) incurred_count
    ,round(openidl_ep_9990.tmp_au_incurred_loss('2000-01-01'::DATE, '2002-01-01'::DATE,a.reporting_code)) incurred_loss
    from openidl_ep_9990.tmp_au_coverage_ref a; 
