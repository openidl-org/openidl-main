...
export class DataService {
  ...
  getDataByStatus(uri: string, status: string): Observable<any> {
    this.getToken();
      return this.http.get(this.API_ENDPOINT + uri + status, this.httpOptions)
      .pipe(map((response: Response) => {

           const serviceResponse = response;
          console.log('statuscode', serviceResponse['success']);
          if (serviceResponse && serviceResponse.status === 200) { // serviceResponse['success'] === true
  ...
  getData(uri) {
    this.getToken();
    return this.http.get(this.API_ENDPOINT + uri, this.httpOptions)
          .pipe(map((response: Response) => {
  ...
  deleteData(uri) {
    this.getToken();
    return this.http.delete(this.API_ENDPOINT + uri, this.httpOptions)
          .pipe(map((response: Response) => {
  ...
  postData(uri, requstObject: any): Observable<number> {
    this.getToken();
    return this.http.post(this.API_ENDPOINT + uri, JSON.stringify(requstObject), this.httpOptions)
        .pipe(map((response: Response) => {
            ...
            if (serviceResponse && serviceResponse['success'] === true) {
                ...
                return serviceResponse['message'] || serviceResponse['result'];
            } else {
...
putData(uri, requstObject: any): Observable<number> {
  this.getToken();
  return this.http.put(this.API_ENDPOINT + uri, JSON.stringify(requstObject), this.httpOptions)
      .pipe(map((response: Response) => {
...
postMultipart(formData: FormData) {
  this.getToken();
   return this.http.post(this.API_ENDPOINT + 'file', formData, this.httpOptionsMultipart)
...
private extractData(res: Response) {
  ...
  *
  */
  private handleError(error: Response | any): ObservableInput<any> {
