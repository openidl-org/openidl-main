...
function sumOutstanding(records){
    ...
    for (let record of records){
        ...
        if (!present){
            ...
            resultSet[record.Claim.OccurrenceIdentifier] = {"accountingDate": record.Policy.AccountingDate, "loss": record.Claim.LossAmount}
        }else{
            //OccurrenceIdentifier already exists, check to make sure we have the most recent version
            if (record.Policy.AccountingDate > resultSet[record.Claim.OccurrenceIdentifier]){
                resultSet[record.Claim.OccurrenceIdentifier] = {"accountingDate": record.Policy.AccountingDate, "loss": record.Claim.LossAmount}
            }
        }
    }
...
async function getIncurredLoss(start,end,coverageCode){
    ...
    let q1 = {$and: [{"TransactionCode": "2"},{"Claim.AccidentDate": {$gte: start}},{"Claim.AccidentDate": {$lte: end}}]}
    
    //get all outstanding loss within time limits
    let q2 = {$and: [{"TransactionCode": "3"},{"Claim.AccidentDate": {$gte: start}},{"Claim.AccidentDate": {$lte: end}}]}
    ...
    console.log('\n\n\n\n   Paid Loss: '+paidLoss+'\n   Outstanding Loss: '+ outstandLoss+'\n   Incurred Loss:'+ incurredLoss)
}
