select q.policy_number,q.state_code, q.state_name, q.territory_key,q.zip_code, q.year, 
       case when q.coverage_code = '1' then 'Bodily Injury'
              ...
              coalesce(q.earned_premium, 0) as earned_premium, 
              coalesce(q.paid_losses, 0) as paid_losses, 
              coalesce(q.paid_losses, 0) + coalesce(q.outstanding_losses, 0) as incurred_losses, 
       coalesce(q.outstanding_losses, 0) as outstanding_losses, 
       coalesce(q.earned_exposures, 0)/24 as earned_exposures,
       coalesce(q.paid_claim_counts, 0) as paid_claim_counts, 
       coalesce(q.incurred_claim_counts, 0) as incurred_claim_counts, 
       coalesce(q.outstanding_claim_counts, 0) as oustanding_claim_counts  ,
       case when (coalesce(q.paid_losses, 0) + coalesce(q.outstanding_losses, 0)) > 25000 then 25000 else (coalesce(q.paid_losses, 0) + coalesce(q.outstanding_losses, 0)) end as basic_limit_il,
       case when (coalesce(q.paid_losses, 0) + coalesce(q.outstanding_losses, 0)) <= 25000 then 0 else (coalesce(q.paid_losses, 0) + coalesce(q.outstanding_losses, 0)) - 25000 end as excess_limit_il   
...
select geo.state_code, geo.state_name, frs.territory_key, geo.zip_code, cov.coverage_code, ded.deductible_amount, ad.year, d.year as accounting_year,
         ...
         frs.incurred_claim_counts, frs.outstanding_claim_counts, frs.policy_number
