...
function makeCoverages(idmPolicy) {
...
async function appendCoverage(dbManager, idmRecord, policyRecord) {
	...
	if (personalAutoPresent) {
		...
		//check coverage
		let coverage_present = checkPersonalAutoCoverage(
			policyRecord,
			newCoverageCode
		); //start from here,
		...
		if (coverage_present) {
			...
			if (policyRecord['PersonalAuto']['Coverages'][newCoverageCode]) {
				...
				let stop = true;
				let IDMcoverageRecords =
					coverage[newCoverageCode].CoverageRecords;
				let policyCoverageRecords =
					policyRecord['PersonalAuto']['Coverages'][newCoverageCode]
						.CoverageRecords;
				...
				let present = false;
				if (
					JSON.stringify(IDMcoverageRecords) ==
					JSON.stringify(policyCoverageRecords)
				) {
					//duplicate pass
					...
					console.log(
						'append to existing coverage: ' + newCoverageCode
					);
					...
					for (let lcl_record of policyCoverageRecords) {
						...
						if (
							JSON.stringify(IDMcoverageRecord) ==
							JSON.stringify(lcl_record)
						) {
							present = true;
						}
					}
					if (!present) {
						policyRecord['PersonalAuto']['Coverages'][
							newCoverageCode
						].CoverageRecords.push(IDMcoverageRecord);
					}
				}
			}
		}
