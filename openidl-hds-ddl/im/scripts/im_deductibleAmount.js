...
let codeMap = require('../codes/im_deductibleAmountCodes.json');
let fileLines = [];
let tableDDL = `
DO $$ 
BEGIN
...
for (let key in flatCodes){
    ...
    line = `    INSERT INTO im_deductible_amount_code VALUES(${index},'${flatCode.code}','${flatCode.description}','${flatCode.type}');`
    fileLines.push(line)
    index+=1
}
...
for (let key in percentageCodes){
  ...
  line = `    INSERT INTO im_deductible_amount_code VALUES(${index},'${percentageCode.code}','${percentageCode.description}','${percentageCode.type}');`
  fileLines.push(line)
  index+=1
}
...
var file = fs.createWriteStream(
	'../tables/V0.0.1.8.6__im_deductible_amount_code.sql'
);
