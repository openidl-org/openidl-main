...
FROM openidl_base_9999.dp_stat_stg a
	LEFT JOIN state_code b 
		ON a.state = b.code
	LEFT JOIN dp_area_indicator_code c 
		ON a.area = c.code
	LEFT JOIN dp_transaction_code d 
		ON a.trans = d.code
	LEFT JOIN dp_annual_statement_line_of_business_code e 
		ON a.aslob = e.code 
	LEFT JOIN dp_program_code f 
		ON a.prog_cd = f.code
	LEFT JOIN dp_major_peril_code g 
		ON a.major_peril = g.code 
	LEFT JOIN dp_coverage_code h
		ON a.cov_code = h.code
	LEFT JOIN dp_terrorism_indicator_code i 
		ON a.terrorism = i.code
	LEFT JOIN dp_deductible_type_code j 
		ON a.ded_type = j.code 
	LEFT JOIN dp_deductible_amount_code k 
		ON a.ded_amt = k.code
	LEFT JOIN dp_windstorm_or_hail_deductible_code l 
		ON a.wind_ded = l.code
	LEFT JOIN dp_class_code m 
		ON a.class_code = m.code 
	LEFT JOIN dp_construction_code n
		ON a.construction = n.code
	LEFT JOIN dp_fire_protection_code o 
		ON a.fire_prot = o.code 
	LEFT JOIN dp_occupancy_code p 
		ON a.occupancy = p.code 
	LEFT JOIN dp_number_of_families_code q 
		ON a.num_fam = q.code
	LEFT JOIN dp_risk_type_code r 
		ON a.risk_type = r.code 
	LEFT JOIN dp_package_identification_code s
		ON a.package_id = s.code
	LEFT JOIN dp_pool_code t 
		ON a.pool = t.code
	LEFT JOIN dp_cause_of_loss_code u 
		ON a.col = u.code;
